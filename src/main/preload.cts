...
import type { CredentialProfile, Session, RdpEmbeddedCapability, RdpLaunchResult, SessionImportExport, SftpEntry, VaultFile } from '../shared/types.js';
...
contextBridge.exposeInMainWorld('api', {
  ...
  exportFullVault: (path: string) => ipcRenderer.invoke('vault:export-full', path),
  exportSelectedSessions: (path: string, sessionIds: string[]) => ipcRenderer.invoke('vault:export-selected-sessions', path, sessionIds),
  importSessions: (path: string) => ipcRenderer.invoke('vault:import', path) as Promise<SessionImportExport>,
  importMergeVault: (path: string) => ipcRenderer.invoke('vault:import-merge', path) as Promise<VaultFile>,
  importReplaceVault: (path: string) => ipcRenderer.invoke('vault:import-replace', path) as Promise<VaultFile>,
  ...
  getRdpEmbeddedCapability: () => ipcRenderer.invoke('rdp:embedded-capability') as Promise<RdpEmbeddedCapability>,
